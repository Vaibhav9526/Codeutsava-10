<script>
    ...
    import { bliss_wallpaper, wallpapers_folder, desktop_folder, SortOptions, SortOrders } from '../../lib/system';
    ...
    async function load_hard_drive(){
        ...
        let drive_changed = prune_removed_programs(hard_drive);
        drive_changed = seed_new_programs(hard_drive) || drive_changed;
        if(drive_changed){
            //write back immediately, otherwise the pruned items reappear on the
            //desktop until the next hardDrive change triggers a save.
            await set('hard_drive', hard_drive);
        }
    ...
    //Programs that no longer ship with win32.run. Hard drives are persisted in
    //IndexedDB and never re-seeded from hard_drive.json, so drives saved before
    //these were removed still hold their desktop shortcuts. Without this the
    //shortcuts linger with a missing icon and do nothing when opened.
    let removed_programs = [
        './programs/foxit_reader.svelte',
        './programs/koodo.svelte',
        './programs/photon.svelte',
        './programs/flash_player.svelte'
    ];

    function prune_removed_programs(drive){
        let removed_ids = Object.keys(drive)
            .filter(key => removed_programs.includes(drive[key]?.url));

        if(removed_ids.length == 0) return false;

        for(let id of removed_ids){
            delete drive[id];
        }
        //drop the dangling child references the deleted items left behind
        for(let key of Object.keys(drive)){
            let obj = drive[key];
            if(obj.children == null) continue;
            obj.children = obj.children.filter(child_id => !removed_ids.includes(child_id));
        }
        console.log('pruned removed programs', removed_ids);
        return true;
    }

    //Desktop shortcuts added after a drive was already persisted to IndexedDB.
    //hard_drive.json is only read on a fresh drive, so returning users would
    //never see these without seeding them here.
    let new_programs = [
        {
            id: 'tfRmK7pQ2sVbN4xLzYw8Ha',
            type: 'file',
            basename: 'Team Formation',
            name: 'Team Formation.exe',
            storage_type: 'fake',
            url: './programs/team_formation.svelte',
            ext: '.exe',
        ...
        {
            id: 'pzWq3Nn8Lk5Rt7YvBx2Cdm',
            type: 'file',
            basename: 'Prizes',
            name: 'Prizes.exe',
            storage_type: 'fake',
            url: './programs/prizes.svelte',
            ext: '.exe',
            parent: desktop_folder,
            size: 2870,
            executable: true,
        ...
        {
            id: 'tlMv6Jq2Zc8Bn4XsRw9Ktp',
            type: 'file',
            basename: 'timeline',
            name: 'timeline.png',
            storage_type: 'remote',
            url: '/files/timeline.png',
            ext: '.png',
            parent: desktop_folder,
            size: 439002,
    ...
    function seed_new_programs(drive){
        let changed = false;

        for(let program of new_programs){
            if(drive[program.id] != null) continue;
            //nothing to attach to if the parent folder itself is gone
            if(drive[program.parent] == null) continue;

            drive[program.id] = {...program};
            drive[program.parent].children = [...drive[program.parent].children, program.id];
            changed = true;
            console.log('seeded new program', program.name);
        }

        return changed;
    }

    function migrate_files_format(drive){        let now = (new Date()).getTime();
        for(let key of Object.keys(drive)){
            let obj = drive[key];
