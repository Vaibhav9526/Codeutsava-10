<script>
    ...
    let iframe;
    let address_bar;
    let default_homepage = 'https://app.usepanda.com/#/';
    let homepage = url ? url : default_homepage;
    ...
    async function load_page(){
        ...
        if(/^[A-Z]:\\/.test(url)){
            ...
            //a page served out of static/, keep it as-is
    ...
    <div slot="content" class="absolute inset-1 top-0 flex flex-col bg-xp-yellow">
        ...
        <div class="shrink-0 flex flex-row items-center border-b border-stone-300 overflow-hidden">
            ...
            <RButton icon="/images/xp/icons/IEHome.png"
                on_click={() => {
                    address_bar.value = default_homepage;
                    load_page();
                }}>
            </RButton>
