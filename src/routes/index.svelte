...
<script>
   ...
   onMount(async () => {
    //jQuery & jQuery UI drive window dragging/resizing. The boot manager used to
    //load them, it's no longer in the boot sequence so they're loaded here instead.
    //loadjs is declared globally in src/app.html.
    loadjs([
    ...
    //there's no boot menu to pick the installer from anymore, so always boot
    //straight into XP. Without this a leftover flag traps startup in the installer.
