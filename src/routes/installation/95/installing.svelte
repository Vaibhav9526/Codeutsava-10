<script>
  ...
  onMount(async () => {
    ...
    //autoplay is rejected until the user interacts with the page
    audio_ready.then(() => {
      if(destroyed || music_node == null) return;
      music_node.play().catch((e) => {
        console.log('xp_installing audio blocked', e);
      });
    });
...
<audio src="/audio/xp_installing.mp3" bind:this={music_node} loop></audio>